					...
					enum led_brightness brightness)
{
	...
	queue_work(chip->wqueue, &chip->work_torch);
}
					 ...
					 enum led_brightness brightness)
{
	...
	queue_work(chip->wqueue, &chip->work_flash);
}
					    ...
					    enum led_brightness brightness)
{
	...
	queue_work(chip->wqueue, &chip->work_indicator);
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	chip->pdata = pdata;
	chip->wqueue = create_singlethread_workqueue("lm355x_workqueue");
	if (!chip->wqueue)
		return -ENOMEM;
